import { Product, Clothing, Appliance } from '../../data/products.js'

describe('test suite: Product class', () => {
  const product = new Product({
    id: "e43638ce-6aa0-4b85-b27f-e1d07eb678c6",
    image: "images/products/athletic-cotton-socks-6-pairs.jpg",
    name: "Black and Gray Athletic Cotton Socks - 6 Pairs",
    rating: {
      stars: 4.5,
      count: 87
    },
    priceCents: 1090,
    keywords: [
      "socks",
      "sports",
      "apparel"
    ]
  })

  it('gets the product details', () => {
    expect(product.id).toEqual("e43638ce-6aa0-4b85-b27f-e1d07eb678c6")
    expect(product.name).toEqual("Black and Gray Athletic Cotton Socks - 6 Pairs")
    expect(product.priceCents).toEqual(1090)
  })
  
  it('gets the price in dollars', () => {
    const price = product.getPrice()

    expect(price).toEqual(`$${(10.90).toFixed(2)}`)
  })

  it(`don't add extra info in HTML`, () => {
    const extraInfoHTML = product.extraInfoHTML()

    expect(extraInfoHTML).toEqual('')
  })

  it('gets the stars url', () => {
    const starsURL = product.getStarsUrl()

    expect(starsURL).toContain(`images/ratings/rating-${product.rating.stars * 10}.png`)
  })
}) 

describe('test suite: Clothing class', () => {
  const product = new Clothing({
    id: "83d4ca15-0f35-48f5-b7a3-1ea210004f2e",
    image: "images/products/adults-plain-cotton-tshirt-2-pack-teal.jpg",
    name: "Adults Plain Cotton T-Shirt - 2 Pack",
    rating: {
      stars: 4.5,
      count: 56
    },
    priceCents: 799,
    keywords: [
      "tshirts",
      "apparel",
      "mens"
    ],
    type: "clothing",
    sizeChartLink: "images/clothing-size-chart.png"
  })

  it('gets the product details', () => {
    expect(product.id).toEqual("83d4ca15-0f35-48f5-b7a3-1ea210004f2e")
    expect(product.name).toEqual("Adults Plain Cotton T-Shirt - 2 Pack")
    expect(product.priceCents).toEqual(799)
  })
  
  it('gets the price in dollars', () => {
    const price = product.getPrice()

    expect(price).toEqual(`$${(7.99).toFixed(2)}`)
  })

  it(`adds extra info in HTML`, () => {
    const extraInfoHTML = product.extraInfoHTML()

    expect(extraInfoHTML).toContain(`
      <a href="${product.sizeChartLink}" target="_blank">
        Size chart
      </a>
    `)
  })

  it('gets the stars url', () => {
    const starsURL = product.getStarsUrl()

    expect(starsURL).toContain(`images/ratings/rating-${product.rating.stars * 10}.png`)
  })

  it('haves a additional sizeChartLink property', () => {
    expect(product.sizeChartLink).toEqual("images/clothing-size-chart.png")
  })
})

describe('test suite: Appliance class', () => {
  const appliance = new Appliance({
    id: "54e0eccd-8f36-462b-b68a-8182611d9add",
    image: "images/products/black-2-slot-toaster.jpg",
    name: "2 Slot Toaster - Black",
    rating: {
      stars: 5,
      count: 2197
    },
    priceCents: 1899,
    keywords: [
      "toaster",
      "kitchen",
      "appliances"
    ],
    type: 'appliance',
    instructionsLink: 'images/appliance-instructions.png',
    warrantyLink: 'images/appliance-warranty.png'
  })

  it('gets the product details', () => {
    expect(appliance.id).toEqual("54e0eccd-8f36-462b-b68a-8182611d9add")
    expect(appliance.name).toEqual("2 Slot Toaster - Black")
    expect(appliance.priceCents).toEqual(1899)
  })
  
  it('gets the price in dollars', () => {
    const price = appliance.getPrice()

    expect(price).toEqual(`$${(18.99).toFixed(2)}`)
  })

  it(`adds extra info in HTML`, () => {
    const extraInfoHTML = appliance.extraInfoHTML()

    expect(extraInfoHTML).toContain(`
    <a href="images/appliance-instructions.png" target="_blank">Instructions</a>
    <a href="images/appliance-warranty.png" target="_blank">Warranty</a>
    `)
  })

  it('gets the stars url', () => {
    const starsURL = appliance.getStarsUrl()

    expect(starsURL).toContain(`images/ratings/rating-${appliance.rating.stars * 10}.png`)
  })

  it('haves instructions and warranty', () => {
    expect(appliance.instructionsLink).toEqual('images/appliance-instructions.png')

    expect(appliance.warrantyLink).toEqual('images/appliance-warranty.png')
  })
})